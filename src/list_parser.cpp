...
auto parse_pos_int(std::string_view str) -> std::optional<int> {
    ...
    // Reject anything that isn't pure decimal digits (e.g. "-3" must be invalid).
    for (char chr : str) {
        if (chr < '0' || chr > '9') { return std::nullopt; }
    }
    ...
    -> std::expected<void, std::string>
{
    ...
    if (dash == std::string_view::npos) {
        ...
        if (!num) { return std::unexpected(std::format("invalid field value: {}", token)); }
        if (*num == 0) { return std::unexpected("values may not include zero"); }
    ...
    if (dash == 0) {
        ...
        if (!end) { return std::unexpected(std::format("invalid field value: {}", token)); }
        if (*end == 0) { return std::unexpected("values may not include zero"); }
        for (int idx = 0; idx < *end; ++idx) { cutlist.indices.insert(idx); }
        return {};
    }
    ...
    if (!lnum) { return std::unexpected(std::format("invalid field value: {}", token)); }
    if (*lnum == 0) { return std::unexpected("values may not include zero"); }
    ...
    if (!rnum) { return std::unexpected(std::format("invalid field value: {}", token)); }
    if (*rnum == 0) { return std::unexpected("values may not include zero"); }
    if (*lnum > *rnum) { return std::unexpected("invalid decreasing range"); }
    for (int idx = *lnum - 1; idx < *rnum; ++idx) { cutlist.indices.insert(idx); }
    return {};
}

}  // anonymous namespace

// NOLINTNEXTLINE(misc-use-internal-linkage) -- declared in list_parser.hpp; external linkage required
    ...
    -> std::expected<CutList, std::string>
{
    ...
    // NOLINTNEXTLINE(misc-const-correctness) -- apply_token mutates result via non-const ref
    CutList result;
